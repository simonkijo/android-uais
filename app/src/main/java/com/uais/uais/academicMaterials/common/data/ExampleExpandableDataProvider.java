...
package com.uais.uais.academicMaterials.common.data;
...
public class ExampleExpandableDataProvider extends AbstractExpandableDataProvider {
    ...
    public ExampleExpandableDataProvider(String groupItems[], String childItems[][]) {
        ...
        for (int i = 0; i < groupItems.length; i++) {
            ...
            final ConcreteGroupData group = new ConcreteGroupData(groupId, groupItems[i]);
            final List<ChildData> children = new ArrayList<>();

            for (int j = 0; j < childItems[i].length; j++) {
                final long childId = j;
                if(childItems[i][j] != null) {
                    children.add(new ConcreteChildData(childId, childItems[i][j]));
                }
            }
